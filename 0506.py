...
class Solution:
    def findRelativeRanks(self, score: list[int]) -> list[str]:
        ...
        n = len(score)
        sorted_scores = [(score[i], i) for i in range(n)]
        sorted_scores.sort(reverse=True)

        ans = [""] * n
        ans[sorted_scores[0][1]] = "Gold Medal"

        if n > 1:
            ans[sorted_scores[0][1]] = "Silver Medal"
        if n > 2:
            ans[sorted_scores[0][1]] = "Bronze Medal"

        for i in range(3, n):
            ans[sorted_scores[i][1]] = str(i + 1)
